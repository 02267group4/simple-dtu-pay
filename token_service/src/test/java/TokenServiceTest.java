...
class TokenServiceTest {
    ...
    void issueToken_storesAndReturnsToken() {
        ...
        String bankAccountId = "bank-1";
        String token = service.issueToken(customerId, bankAccountId);
        assertNotNull(token);
        verify(repo, times(1)).store(eq(token), eq(customerId), eq(bankAccountId));
    }

    @Test
    void issueTokens_invalidCount_throws() {
        assertThrows(TokenService.InvalidTokenException.class, () -> service.issueTokens("c", "bank", 0));
        assertThrows(TokenService.InvalidTokenException.class, () -> service.issueTokens("c", "bank", 6));
    }

    @Test
    void issueTokens_respectsExistingLimits() throws Exception {
        when(repo.findByCustomerId("c")).thenReturn(List.of());
        List<String> tokens = service.issueTokens("c", "bank", 3);
        assertEquals(3, tokens.size());
        verify(repo, times(3)).store(anyString(), eq("c"), eq("bank"));
    }

    @Test
    void issueTokens_tooManyExisting_throws() {
        when(repo.findByCustomerId("c")).thenReturn(List.of("a","b"));
        assertThrows(TokenService.InvalidTokenException.class, () -> service.issueTokens("c", "bank", 1));
    }

    @Test
    void consumeToken_success_returnsTokenInfo() throws Exception {
        TokenInfo info = new TokenInfo("cust", "bank");
        when(repo.consume("t")).thenReturn(Optional.of(info));
        TokenInfo result = service.consumeToken("t");
        assertEquals("cust", result.customerId());
        assertEquals("bank", result.bankAccountId());
